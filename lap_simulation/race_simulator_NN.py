...
class RaceSimulator:
    def __init__(self, model, preprocessor, model_type='lstm'):
        self.model = model
        self.preprocessor = preprocessor
        self.model_type = model_type
        self.race_features = RaceFeatures()
        self.pit_stop_duration = 0  # Average pit stop duration in milliseconds
    ...
    def simulate_driver_lap(self, driver, lap, race):
        ...
        """
        # Temporarily revert TrackStatus and is_pit_lap to their defaults for prediction
        original_track_status = driver.dynamic_features['TrackStatus']
        original_is_pit_lap = driver.dynamic_features['is_pit_lap']
        
        driver.dynamic_features['TrackStatus'] = 1  # Default track status
        driver.dynamic_features['is_pit_lap'] = 0  # Default pit lap status

        # Prepare sequence and static features for prediction
        ...
        # Restore the original TrackStatus and is_pit_lap values
        driver.dynamic_features['TrackStatus'] = original_track_status
        driver.dynamic_features['is_pit_lap'] = original_is_pit_lap

        # Apply manual adjustments for safety car or pit stop
        if original_track_status == 4:
            lap_time *= 1.1  # Increase lap time by 30% for safety car
